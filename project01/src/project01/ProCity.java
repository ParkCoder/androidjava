...
public class ProCity {
	...
	public static void main(String[] args) throws IOException {
		...
		String date1 = date.get(0).text(); //date에서 크롤링 하여 가져온 텍스트

		String[] city = new String[nujuck_list.size()]; //크롤링으로 추출한 누적확진자 텍스트 배열
		String[] city_day = new String[yseter_list.size()]; //크롤링으로 추출한 전일대비 확진자 텍스트 배열

		for (int i = 0; i < nujuck_list.size(); i++) { //for문으로 반복하여 지역별 누적확진자 텍스트를 배열에 넣는다
			String nujuck = nujuck_list.get(i).text();
			city[i] = nujuck;
		}
		for (int i = 0; i < yseter_list.size(); i++) { //for문으로 반복하여 지역별 전일대비 확진자 텍스트를 배열에 넣는다
